...
export async function listAccounts(): Promise<Account[]> {
  ...
  return (await db.getAllAsync(`SELECT * FROM accounts ORDER BY created_at ASC`)) as Account[];
}
...
export async function upsertAccount(a: Omit<Account, 'id'> & { id?: string }): Promise<Account> {
  ...
  const id = a.id ?? genId();
  await db.runAsync(
    `INSERT OR REPLACE INTO accounts (id, name, type, account_no, notes)
     VALUES (?, ?, ?, ?, ?)`,
    [id, a.name, a.type, a.account_no ?? null, a.notes ?? null]
  );
  return { ...a, id };
}
